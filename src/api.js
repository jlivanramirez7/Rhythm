...
const getFilledCycle = async (cycleId, db) => {
  ...
  if (cycle.end_date) {
    ...
    if (cycleEndDate.isAfter(lastDate)) {
      ...
      log(
        "debug",
        `[GET_FILLED] Updated lastDate based on cycle.end_date: ${lastDate.format()}`
      );
    }
  }
  ...
  if (days.length > 0) {
    ...
    if (lastReadingDate.isAfter(lastDate)) {
      ...
      log(
        "debug",
        `[GET_FILLED] Updated lastDate based on last reading: ${lastDate.format()}`
      );
    }
  }
  ...
  log(
    "debug",
    `[GET_FILLED] Loop will run from ${currentDate.format()} until ${lastDate.format()}`
  );
  while (currentDate.isSameOrBefore(lastDate)) {
    ...
    if (existingDay) {
      ...
      log(
        "debug",
        `getFilledCycle: Found existing day in map for ${dateStr}. Pushing data:`,
        existingDay
      );
...
const upsertReading = async (db, data) => {
  ...
  const findExistingSql = sql(
    `SELECT * FROM cycle_days WHERE cycle_id = ? AND date = ?`,
    isPostgres
  );
...
const apiRouter = (db) => {
  ...
  router.post("/instructions-viewed", async (req, res) => {
    ...
    try {
      await db.run(
        sql("UPDATE users SET show_instructions = false WHERE id = ?", isPostgres),
        [userId]
      );
  ...
  router.put("/settings", async (req, res) => {
    ...
    console.log(
      `[API] Received PUT /api/settings for user ${userId}. show_instructions: ${show_instructions}`
    );
    ...
    try {
      const result = await db.run(
        sql("UPDATE users SET show_instructions = ? WHERE id = ?", isPostgres),
        [show_instructions, userId]
      );
  ...
  router.get("/shared-users", async (req, res) => {
    ...
    try {
      ...
      const users = await db.query(
        sql("SELECT id, name, email FROM users WHERE partner_id = ? OR id = ?", isPostgres),
        [userId, userId]
      );
      res.json(users);
    } catch (err) {
  ...
  router.post("/partner", async (req, res) => {
    ...
    try {
      ...
      const partner = await db.get(
        sql("SELECT id FROM users WHERE email = ?", isPostgres),
        [email]
      );
      ...
      if (!partner) {
        return res
          .status(404)
          .json({ error: "Partner not found. Please ensure they have registered." });
      }

      await db.run(sql("UPDATE users SET partner_id = ? WHERE id = ?", isPostgres), [
        partner.id,
        userId
      ]);
  ...
  router.post("/cycles", async (req, res) => {
    ...
    if (targetUserId !== requestingUserId) {
      const partnerCheck = await db.get(
        sql("SELECT id FROM users WHERE id = ? AND partner_id = ?", isPostgres),
        [targetUserId, requestingUserId]
      );
      if (!partnerCheck) {
        return res
          .status(403)
          .json({ error: "Forbidden: You do not have permission to create a cycle for this user." });
      }
    }
    ...
    try {
      ...
      const findPreviousCycleSql = sql(
        `SELECT id FROM cycles WHERE user_id = ? AND start_date < ? ORDER BY start_date DESC LIMIT 1`,
        isPostgres
      );
      log(
        "info",
        `[NEW_CYCLE_LOGIC] Find Previous SQL: ${findPreviousCycleSql} | Params: [${targetUserId}, ${start_date}]`
      );
      ...
      if (previousCycle) {
        const previousCycleEndDate = moment
          .utc(start_date)
          .subtract(1, "days")
          .format("YYYY-MM-DD");
        log(
          "info",
          `[NEW_CYCLE_LOGIC] Previous cycle ${previousCycle.id} found. Calculated end date: ${previousCycleEndDate}.`
        );

        const updatePreviousCycleSql = sql(
          `UPDATE cycles SET end_date = ? WHERE id = ?`,
          isPostgres
        );
        log(
          "info",
          `[NEW_CYCLE_LOGIC] Update Previous SQL: ${updatePreviousCycleSql} | Params: [${previousCycleEndDate}, ${previousCycle.id}]`
        );
        const updateResult = await db.run(updatePreviousCycleSql, [
      ...
      const insertCycleSql = sql(
        `INSERT INTO cycles (user_id, start_date) VALUES (?, ?)`,
        isPostgres
      );
      const result = await db.run(insertCycleSql, [targetUserId, start_date]);
      const newCycleId = result.lastID;

      const insertDaySql = sql(
        `INSERT INTO cycle_days (cycle_id, date, hormone_reading) VALUES (?, ?, ?)`,
        isPostgres
      );
      for (let i = 0; i < 5; i++) {
  ...
  router.post("/cycles/days/range", async (req, res) => {
    log(
      "info",
      `[API] POST /api/cycles/days/range - Request received for user ${req.user.id}.`
    );
    log("debug", "[API] Request body:", req.body);
        ...
        if (cycle) {
          ...
          if (targetUserId !== requestingUserId) {
            const owner = await db.get(
              sql("SELECT partner_id FROM users WHERE id = ?", isPostgres),
              [targetUserId]
            );
            if (!owner || owner.partner_id !== requestingUserId) {
              log(
                "warn",
                `Forbidden attempt by user ${requestingUserId} to edit data for user ${targetUserId}.`
              );
    ...
    try {
      ...
      if (targetUserId !== requestingUserId) {
        ...
        if (!partnerCheck) {
          return res
            .status(403)
            .json({ error: "Forbidden: You do not have permission to edit data for this user." });
        }
      }
  ...
  router.get("/analytics", async (req, res) => {
    ...
    try {
      ...
      if (cycleLengths.length > 0) {
        ...
        analytics.averageCycleLength = Math.round(
          totalDays / cycleLengths.length
        );
      } else {
      ...
      if (peakRows.length > 0) {
        ...
        analytics.averageDaysToPeak = Math.round(
          totalDaysToPeak / peakRows.length
        );
      } else {
  ...
  router.delete("/cycles/:id", async (req, res) => {
    log(
      "info",
      `[API] DELETE /api/cycles/${req.params.id} - Request received from user ${req.user.id}.`
    );
    const { id } = req.params;
    const requestingUserId = req.user.id;
    ...
    try {
      ...
      const cycle = await db.get(sql("SELECT user_id FROM cycles WHERE id = ?", isPostgres), [
        id
      ]);
      if (!cycle) {
      ...
      const owner = await db.get(
        sql("SELECT id, partner_id FROM users WHERE id = ?", isPostgres),
        [cycle.user_id]
      );
      if (!owner || (owner.id !== requestingUserId && owner.partner_id !== requestingUserId)) {
        log(
          "warn",
          `[API] Forbidden attempt by user ${requestingUserId} to delete cycle ${id} owned by user ${cycle.user_id}.`
        );
        return res
          .status(403)
          .json({ error: "Forbidden: You do not have permission to delete this cycle." });
      }
  ...
  router.put("/cycles/days/:id", async (req, res) => {
    log(
      "info",
      `[API] PUT /api/cycles/days/${req.params.id} - Request received for user ${req.user.id}.`
    );
    log("debug", "[API] Request body:", req.body);
    const { id } = req.params;
    ...
    try {
      ...
      const existingReading = await db.get(
        sql("SELECT * FROM cycle_days WHERE id = ?", isPostgres),
        [id]
      );
      ...
      // 2. Merge the changes. Start with existing data.
      const updatedReading = { ...existingReading };

      // If a new hormone_reading is provided, update it.
      if (hormone_reading !== undefined) {
        updatedReading.hormone_reading =
          hormone_reading === "" ? null : hormone_reading;
      }
      // If a new intercourse status is provided, update it.
      if (intercourse !== undefined) {
        updatedReading.intercourse = intercourse;
      }

      // 3. Save the complete, merged record.
      const updateSql = sql(
        "UPDATE cycle_days SET hormone_reading = ?, intercourse = ? WHERE id = ?",
        isPostgres
      );
      const result = await db.run(updateSql, [
        updatedReading.hormone_reading,
        updatedReading.intercourse ? 1 : 0,
        id
      ]);
  ...
  router.delete("/cycles/days", async (req, res) => {
    const { dayIds } = req.body;

    if (!dayIds || !Array.isArray(dayIds) || dayIds.length === 0) {
      return res.status(400).json({ error: "Invalid or empty dayIds array" });
    }
    ...
    try {
      ...
      await db.run(deleteSql, dayIds);
      res.status(204).send();
    } catch (err) {
      log("error", "Error in DELETE /api/cycles/days:", err);
      res
        .status(500)
  ...
  router.delete("/cycles/days/:id", async (req, res) => {
    ...
    const { id } = req.params;
    try {
      const result = await db.run(sql(`DELETE FROM cycle_days WHERE id = ?`, isPostgres), [
        id
      ]);
      if (result.changes === 0) {
        return res.status(404).send("Reading not found.");
  ...
  router.delete("/data", async (req, res) => {
    ...
    try {
      const cycles = await db.query(
        sql(`SELECT id FROM cycles WHERE user_id = ?`, isPostgres),
        [userId]
      );
      ...
      if (cycleIds.length > 0) {
        ...
        await db.run(
          sql(`DELETE FROM cycle_days WHERE cycle_id IN (${placeholders})`, isPostgres),
          cycleIds
        );
        await db.run(sql(`DELETE FROM cycles WHERE user_id = ?`, isPostgres), [
          userId
        ]);
      }
      res.status(204).send();
