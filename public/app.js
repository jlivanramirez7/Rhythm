const app = {
  ...
  fetchAndRenderData: async () => {
    try {
      ...
      const cyclesRes = await fetch('/api/cycles');
      const cycles = await cyclesRes.json();
      app.renderCycles(cycles);

      // Fetch analytics
      const analyticsRes = await fetch('/api/analytics');
      const analytics = await analyticsRes.json();
      app.renderAnalytics(analytics, cycles);
    } catch (error) {
      console.error('Error fetching data:', error);
    }
  },
  renderCycles: (cycles) => {
        ...
        cycles.forEach((cycle, index) => {
            ...
            menuButton.className = 'menu-button';
            menuButton.innerHTML = `<span></span><span></span><span></span>`;
            menuButton.onclick = () => app.toggleEditMode(cycleDiv, cycle.id);
            cycleDiv.appendChild(menuButton);
            ...
            for (let i = 0; i < totalDays; i++) {
                ...
                const { fertileWindows } = app.calculateFertileWindows(cycles);
                const cycleFertileWindow = fertileWindows[index];
                if (cycleFertileWindow && dayDate >= cycleFertileWindow.start && dayDate <= cycleFertileWindow.end) {
                    dayDiv.classList.add('fertile-window');
                }
    ...
    calculateFertileWindows: (cycles) => {
        let totalFertileDays = 0;
        let fertileCyclesCount = 0;
    ...
    renderAnalytics: (analytics, cycles) => {
        ...
        const { averageFertileWindow, fertileWindows } = app.calculateFertileWindows(cycles);
        document.getElementById('avg-fertile-window').textContent = averageFertileWindow > 0 ? `${averageFertileWindow} days` : '--';
    ...
    toggleEditMode: (cycleDiv, cycleId) => {
        const isEditing = cycleDiv.classList.toggle('edit-mode');
        const menuButton = cycleDiv.querySelector('.menu-button');
        ...
        dayDivs.forEach(dayDiv => {
            ...
            if (readingDiv) {
                if (isEditing) {
                    ...
                    if (changesMade) {
                        app.updateReading(dayData.id, payload);
                    }
